...
def get_useful_xml_elements(tree, df):
    ...
    if total_car_count == 0:
        total_car_count = parse_numeric(
            ''.join(tree.xpath("//div[contains(@class,'max results')]/h1/text()")).split(' ')[0])
    ...
    for element in tree.xpath("//div[contains(@class, 'listing-item')]"):
        ...
        title_string = element.xpath(
            "div[contains(@class, 'listing-header')]/div[contains(@class, 'n_columns')]/div[contains(@class, 'n_width-max title ')]/a/h2/text()")
        price_string = element.xpath(
            "div[contains(@class, 'listing-body')]/div[contains(@class,'n_columns')]/div[contains(@class, 'price')]" +
        ...
        link_string = element.xpath(
            "div[contains(@class, 'listing-header')]/div[contains(@class, 'n_columns')]/div[contains(@class, 'n_width-max title ')]/a/@href")

        price = parse_numeric(''.join(price_string))
        ...
        if len(title_string) > 0 and price > 0:
            ...
            milage = parse_numeric(''.join(milage_string).strip())
...
def main():
    ...
    ######################################################################################
    print 'Processed first page: ' + url
    ...
    df = pagination_scrape(page_loop_counter=0, url=url, pagination_offset=60, doc_count=total_car_count,
                           LIVE_DATA=not USE_LOCAL_COPY, df=df)
    ...
    if db_save:
        save_to_db()
    save_output_files(df, weightings, extpath='../../../willzjc.github.io/valuerating/',
